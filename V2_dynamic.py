...
class FootballTrader:
    ...
    def format_nonce_for_aa(self, api_nonce: int) -> str:
        """
        Преобразование nonce из API в формат Account Abstraction
        
        Из успешного curl видно, что nonce имеет формат: 0x198eb498bc70000000000000000
        Это комбинированный nonce для AA кошельков
        """
        # Базовая часть для AA кошелька (из успешного curl)
        base_part = "0x198eb498bc7"
        
        # Добавляем API nonce в hex формате, дополненный нулями
        api_nonce_hex = f"{api_nonce:x}".zfill(15)  # 15 символов для дополнения
        
        formatted_nonce = base_part + api_nonce_hex
        print(f"✅ Nonce преобразован: API {api_nonce} -> AA {formatted_nonce}")
        return formatted_nonce

    def format_signature_for_aa(self, api_signature: str) -> str:
        """
        Преобразование подписи из API в формат Account Abstraction
        """
        print("🔧 Преобразуем подпись для AA кошелька...")
        ...
        # Добавляем длину подписи (в hex, 32 байта)
        ...
        print(f"✅ Подпись преобразована: {len(aa_signature)} символов")
        return aa_signature

    def get_paymaster_data(self, calldata: str, api_nonce: int, api_signature: str, deadline: int) -> str:
        """Получение актуальных paymaster данных"""
        print("📤 Получаем paymaster данные...")
        
        # Преобразуем nonce и signature в формат AA
        formatted_nonce = self.format_nonce_for_aa(api_nonce)
    ...
    def send_transaction(self, calldata: str, paymaster_data: str, api_nonce: int, api_signature: str) -> Dict:
        """Отправка транзакции"""
        print("📤 Отправляем транзакцию...")
        
        # Преобразуем данные в формат AA
        formatted_nonce = self.format_nonce_for_aa(api_nonce)
        formatted_signature = self.format_signature_for_aa(api_signature)
        
        # Финальный UserOperation с динамическими данными
        user_op = {
            "callData": calldata,
            ...
            "sender": "0x8f37a8015851976aB75E309100c2511abaBC68AD",
            "signature": formatted_signature,  # Динамическая подпись из API
    ...
    def buy_player(self, player_id: int, shares: int = 1) -> Dict:
        ...
        try:
            ...
            paymaster_data = self.get_paymaster_data(calldata, nonce, signature, deadline)
            ...
            result = self.send_transaction(calldata, paymaster_data, nonce, signature)
