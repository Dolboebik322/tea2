...
class FootballTrader:
    ...
    def get_nonce_from_blockchain(self) -> str:
        ...
        print(f"✅ Blockchain nonce: {nonce_result}")
        return nonce_result
    ...
    def buy_player_exact(self, player_id: int, shares: int = 1) -> Dict:
        ...
        try:
            ...
            # 3. Получаем nonce из блокчейна
            print("\n3️⃣ Получаем nonce из блокчейна...")
            blockchain_nonce = self.get_nonce_from_blockchain()
            ...
            # 7. Получаем paymaster данные
            print("\n7️⃣ Получаем paymaster данные...")
            paymaster_data = self.get_paymaster_stub_data(base_user_op)
