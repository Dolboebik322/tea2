...
class FootballTrader:
    ...
    def get_paymaster_data(self, user_op: Dict) -> str:
        ...
        # Правильный chain ID для Base mainnet
        payload = {
            "jsonrpc": "2.0",
            "id": 1,
            "method": "pm_getPaymasterStubData",
            "params": [user_op, self.config.entry_point_address, "base", {}]
        }
    ...
    def buy_player_complete(self, player_id: int, shares: int = 1) -> Dict:
        ...
        try:
            ...
            # 7. Получаем stub данные paymaster
            print("\n7️⃣ Получаем stub данные paymaster...")
            stub_paymaster_data = self.get_paymaster_data(base_user_op)
            base_user_op["paymasterAndData"] = stub_paymaster_data
            
            # 8. Получаем финальные данные paymaster для спонсирования
            print("\n8️⃣ Получаем финальные данные paymaster...")
            final_paymaster_data = self.get_paymaster_and_data(base_user_op)
            base_user_op["paymasterAndData"] = final_paymaster_data
