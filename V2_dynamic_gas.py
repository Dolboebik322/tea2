...
class FootballTrader:
    ...
    def get_paymaster_data(self, calldata: str, blockchain_nonce: str, api_signature: str, gas_prices: Dict) -> Optional[str]:
    ...
    def send_user_operation(self, calldata: str, paymaster_data: str, blockchain_nonce: str, api_signature: str, gas_prices: Dict) -> Dict:
    ...
    def buy_player_complete(self, player_id: int, shares: int = 1) -> Dict:
        ...
        try:
            ...
            print("\n4️⃣ Получаем актуальные цены на газ...")
            ...
            paymaster_data = self.get_paymaster_data(calldata, blockchain_nonce, signature, gas_prices)
            
            if not paymaster_data:
                return {"success": False, "error": "Не удалось получить paymaster данные"}
            
            # 7. Отправка UserOperation
            print("\n7️⃣ Отправляем UserOperation...")
            send_result = self.send_user_operation(calldata, paymaster_data, blockchain_nonce, signature, gas_prices)
