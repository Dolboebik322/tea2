...
class FootballTrader:
    ...
    def get_blockchain_nonce(self) -> str:
        """
        Получение актуального nonce из блокчейна через EntryPoint
        """
        print("📤 Получаем актуальный nonce из блокчейна...")
        
        # Вызываем EntryPoint.getNonce(sender, key=0)
        payload = {
            "jsonrpc": "2.0",
            "id": 1,
            "method": "eth_call",
        ...
        response = self.session.post(
            "https://base-mainnet.g.alchemy.com/v2/CCDNKip23OyFJ7ssex2-O",
            headers=self.rpc_headers,
            json=payload
        )
        
        if response.status_code != 200:
            print(f"❌ Ошибка получения nonce: {response.status_code}")
            return "0x0"
        
        result = response.json()
        if "error" in result:
            print(f"❌ RPC ошибка nonce: {result['error']}")
            return "0x0"
        ...
        print(f"✅ Blockchain nonce: {blockchain_nonce} -> {formatted_nonce}")
    ...
    def get_paymaster_data(self, calldata: str, api_signature: str, deadline: int) -> tuple:
        ...
        user_op = {
            ...
            "nonce": blockchain_nonce,  # Актуальный nonce из блокчейна
        ...
        return paymaster_data, blockchain_nonce

    def send_transaction(self, calldata: str, paymaster_data: str, blockchain_nonce: str, api_signature: str) -> Dict:
        """Отправка транзакции"""
        print("📤 Отправляем транзакцию...")
        
        # Преобразуем подпись в формат AA
        formatted_signature = self.format_signature_for_aa(api_signature)
    ...
    def buy_player(self, player_id: int, shares: int = 1) -> Dict:
        ...
        try:
            ...
            paymaster_result = self.get_paymaster_data(calldata, signature, deadline)
            
            if not paymaster_result:
                return {"success": False, "error": "Не удалось получить paymaster данные"}
            
            paymaster_data, blockchain_nonce = paymaster_result
            
            # 5. Отправляем транзакцию
            print("\n5️⃣ Отправляем транзакцию...")
            result = self.send_transaction(calldata, paymaster_data, blockchain_nonce, signature)
